...
class YouTubeLoader:
    def __init__(self, youtube_url, start_time: time = None, end_time: time = None, add_video_info: bool = False, continue_on_failure: bool = False):
    ...
    # Define the filtering function based on the provided min and/or max values
        def is_within_range(d):
            if start is not None and time(d['start']) < time(start):
                return False
            if end is not None and time(d['start']) > time(end):
                return False
            return True
    ...
    def load(self) -> List[Document]:
        ...
        for transcript in filetred_transcrpit_peices:
            transcript = " ".join([transcript["text"].strip(" ")])
        ...
        for upload_file in self.files:
            ...
            with upload_file.file as file:
                if file_type == "pdf":
                    ...
                    page_content = get_text_from_docx(file)
                    metadata = {"source": upload_file.filename}
                    doc = Document(page_content=page_content, metadata=metadata)
                    documents.append(doc)

                elif file_type in ["ppt", "pptx"]:
                    page_content = get_text_from_pptx(file)
                    metadata = {"source": upload_file.filename}
                    doc = Document(page_content=page_content, metadata=metadata)
                    documents.append(doc)
            ...
            try:
                ...
                if path.endswith(expected_file_extensions, "/o/"):
                    if response.status_code == 200:
                        ...
                        if file_type != self.expected_file_types:
                            raise LoaderError(f"Expected file type: {self.expected_file_types}, but got: {file_type}")
